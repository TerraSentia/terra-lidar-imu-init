...
//  rclcpp::Node::SharedPtr nh;
 ...
 private:
  void IMU_init(const MeasureGroup &meas, StatesGroup &state_inout, int &N);
...
{
  RCLCPP_WARN(rclcpp::get_logger("ImuProcess"), "Reset ImuProcess with timestamp and IMU data");
  mean_acc      = V3D(0, 0, -1.0);
  mean_gyr      = V3D(0, 0, 0);
  angvel_last       = Zero3d;
  imu_need_init_    = true;
...
void ImuProcess::propagation_and_undist(const MeasureGroup &meas, StatesGroup &state_inout, PointCloudXYZI &pcl_out)
{
  ...
  double imu_end_time = v_imu.back()->header.stamp.sec + v_imu.back()->header.stamp.nanosec * 1e-9;
...
void ImuProcess::Process(const MeasureGroup &meas, StatesGroup &stat, PointCloudXYZI::Ptr cur_pcl_un_)
{
  if (imu_en)
  {
    ...
    RCLCPP_INFO(rclcpp::get_logger("ImuProcess"), "meas.lidar: %p", meas.lidar.get());
    ...
    if (imu_need_init_)
    {
        if(!LI_init_done){
            ...
            if (init_iter_num > MAX_INI_COUNT)
            {
                ...
                RCLCPP_INFO(rclcpp::get_logger("ImuProcess"), "IMU Initialization Done: Gravity: %.4f %.4f %.4f, Acc norm: %.4f", stat.gravity[0], stat.gravity[1], stat.gravity[2], mean_acc.norm());
